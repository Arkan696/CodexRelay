...
export function ChatScreen({ initialPairingUrl }: ChatScreenProps = {}) {
  ...
  const [scannerMessage, setScannerMessage] = useState(
    translateUiText("Point the camera at the connection QR."),
  );
  ...
  async function openScanner() {
    if (!cameraPermission?.granted) {
      ...
      if (!permission.granted) {
        setScannerMessage(
          translateUiText("Camera access is off. Allow camera access to scan the connection QR."),
        );
        setScannerOpen(true);
        return;
      }
    ...
    setScannerMessage(translateUiText("Point the camera at the connection QR."));
  ...
  const handleScanPayload = useCallback(
    async (payload: unknown) => {
      ...
      setPasteApprovalCode(undefined);
      setPasteApprovalServerUrl(undefined);
      setScannerMessage(translateUiText("QR detected. Pairing..."));
      await closeScannerSurface();
      setPastePairOpen(true);
      try {
...
function approvalMessage(approvalCode: string, serverUrl?: string) {
  return `请在服务器终端中运行 ${approvalCommand(approvalCode, serverUrl)}。`;
}
